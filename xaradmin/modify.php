 ...
 * Modify a response
 *
 * This is a standard function to edit information on a single item
 ...
 */
function sitecontact_admin_modify($args)
{
    extract($args);
    ...
    if (!xarSecurityCheck('DeleteSiteContact',0,'ContactForm',"$scid")) {
   ...
   // Create filters based on publication type
    ...
    $data['returnurl']=xarModURL('sitecontact','admin','view',
                                         array('scid' => $lastview['scid'],
                                               'startnum'=> $lastview['startnum']));
    $templatedata = xarTplModule('sitecontact', 'admin', 'modify', $data, $template);
