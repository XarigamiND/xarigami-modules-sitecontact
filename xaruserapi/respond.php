...
function sitecontact_userapi_respond($args)
{
    ...
    if (!xarUserIsLoggedIn()) { //we want to use captcha else don't bother if user is logged in
    ...
    //Issue Issue sicon-000349 need to set an empty array
    $fieldconfigs    = !empty($fieldconfig) ?explode(',',$fieldconfig) : array();
   ...
   $antibotinvalid = isset($hookinfo['antibotinvalid']) ? $hookinfo['antibotinvalid'] : 0;
    ...
    $data = array('authid'         => xarSecGenAuthKey('sitecontact'),
                      ...
                      'options'        => $options,
                      'optionitems'    => $optionitems, //the request text options exploded (old)
    ...
    if (($adminccs == 1)  && !empty($admincclist)) {
        if (!isset($ccrecipients) || $ccrecipients == '') { //no cc list
            $ccrecipients = $admincclist; //just put the admincclist in the ccrecipients list and process that
            $adminccs = true;
    ...
    if ($withupload && isset($fileuploadfieldname) && is_array($items[$fileuploadfieldname]) && !empty($items[$fileuploadfieldname]['value'])) {
       $filebasepath='';//$items[$fileuploadfieldname]['basePath'];
        $filebasedir=$items[$fileuploadfieldname]['xv_basedir'];
        $fileattachmentname=$items[$fileuploadfieldname]['value'];
        $attachpath=$filebasedir.'/'.$fileattachmentname;//$filebasepath.'/'.$filebasedir.'/'.$fileattachmentname;
    ...
    $newscrid = isset($newscrid)? $newscrid: '';
    ...
    */
    ...
    try {
        ...
        $usertextmessage= xarTplModule('sitecontact', 'user', 'usermail-text',$usertextarray);
    }
    ...
    try {
        ...
        $adminhtmlmessage= xarTplModule('sitecontact', 'user', 'adminmail-html',$adminhtmlarray);
    }
    ...
    try {
        ...
        $admintextmessage= xarTplModule('sitecontact', 'user', 'adminmail-text',$adminhtmlarray);
    }
    /* send email to admin */
